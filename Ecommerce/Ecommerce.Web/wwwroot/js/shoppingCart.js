            ...
            },
            {
                data: 'quantity', className: 'td-number', render: function (data, type, row) {
                    return `<div class="input-group input-group-sm">
                            <input type="text" class="form-control text-center quantity mx-1" style="max-width: 50px; border: 1px solid lightgrey; border-radius: 0;" value="${data}" readonly>
                            <div class="btn-group btn-group-sm m-0">
                                <button class="border-0" onclick="updateQuantity(${row.id}, -1)"> <i class="material-icons grpicon-color">remove</i> </button>
                                <button class="border-0" onclick="updateQuantity(${row.id}, 1)"> <i class="material-icons grpicon-color">add</i> </button>
                            </div>`;
                }
            },
            {
            ...
            },
            {
                data: null, className: 'td-actions', sorting: false, render: function (data, type, row) {
                    return `<button type="button" rel="tooltip" data-placement="left" title="" class="btn btn-link" onclick="removeItem(${row.id})">
                             <i class="material-icons carticon-color">close</i>
                            </button>`;
                }
            }
        ],
